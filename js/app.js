...
function stateFlag(name) {
  return Ember.computed(function() {
    var state = EmRouteApp.router.currentState;
    while(state) {
      if(state.name === name) return true;
      state = state.get('parentState');
    }
    return false;
  }).property('EmRouteApp.router.currentState');
}

// Create the EmRdouteApplication
window.EmRouteApp = Ember.Application.create({


  // Define the main EmRouteApplication controller. This is automatically picked up by
  // the EmRouteApplication and initialized.
  ...
  Router: Ember.Router.extend({
    root: Ember.Route.extend({
      doHome: function(router, event) {
        //EmRouteApp.socket.emit('statechange', { my: 'data' });
